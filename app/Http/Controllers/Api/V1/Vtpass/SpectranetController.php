...
use App\Http\Requests\Vtpass\SpectranetPurchaseRequest;
use App\Models\VtpassTransaction;
use App\Services\Vtpass\SpectranetService;
...
class SpectranetController extends Controller
{
    ...
    public function verify(Request $request)
    {
        if (!$this->isVtpassEnabled()) {
            return $this->error('Spectranet Service Currently Unavailable');
        }

        $validated = $request->validate([
            'serviceID' => 'required|string', // spectranet
            'billersCode' => 'required|string',
        ]);

        try {
            $response = $this->spectranetService->verifyAccount($validated['serviceID'], $validated['billersCode']);
            return $this->ok('Spectranet account verified', $response);
        } catch (\Exception $e) {
    ...
    {
        $serviceID = $request->input('serviceID', 'spectranet');
        try {
            $response = $this->spectranetService->getVariations($serviceID);
    ...
    public function purchase(SpectranetPurchaseRequest $request)
    {
        $user = auth()->user();
        ...
        try {
            debitWallet(
                user: $user,
                amount: $amount,
                serviceName: 'VTpass Spectranet Purchase',
                serviceDesc: "Spectranet purchase for {$validated['billersCode']}",
                transactionRef: $transactionRef,
                wrapInTransaction: false
            );

            $response = $this->spectranetService->purchase(
                $transactionRef,
                $validated['serviceID'],
            ...
            if ($status === TransactionStatus::SUCCESS) {
                return $this->ok('Spectranet purchase successful', $response);
            } else {
                return $this->error('Spectranet purchase failed: ' . ($response['response_description'] ?? 'Unknown Error'), 400, $response);
            }

        } catch (\Exception $e) {
            DB::rollBack();
            Log::error('VTpass Spectranet purchase failed', ['error' => $e->getMessage()]);
    ...
    private function isVtpassEnabled(): bool
    {
        ...
        if ($enabled === null) {
            ...
            $enabled = getConfigValue($config, 'vtpassStatus') === 'On' &&
                getConfigValue($config, 'vtpassSpectranetStatus') === 'On';
        }
